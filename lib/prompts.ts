...
2. 具体可执行：写明「做什么、怎么做、做多久」，不要「多关心对方」这种模糊建议
3. 不评判、不挑刺：避免「你应该」「你不该」「你总是」
4. 共同完成：必须是两个人一起做的一件事，不是话题讨论、不是约定规则、不是沟通技巧练习
5. 贴近关系阶段：${stageLabel}的特征要体现在任务里
6. 不重复老套：不要「一起看电影」「说我爱你」这种泛泛任务

# 关键区分（必须遵守）
- 错误示例（话题/约定型）：听完对方说话再回应、约定一个暂停手势、讨论一个小传统、识别摩擦引爆点
- 正确示例（共同完成型）：一起完成一次3分钟眼神对话、一起做一顿饭、一起给对方一个20秒拥抱、一起完成一次无手机90分钟
